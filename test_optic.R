...
plan(tweak(multiprocess, workers=8))
...
for(r in 1:length(rhos))
{
  ...
  print(rho)
  results <- list()
  for (i in 1:Sim4$iters) {
    results[[i]] <- run_iteration_concurrent(Sim4,rho=rho)
  }
  # combine results together
  full_results <- do.call("rbind", results)
  names(full_results)[14]<-"joint.eff.variance"
  names(full_results)[15]<-"joint.eff.t_stat"
  #then would summarize results and save output for this iteration
  # attached summary information to config object
  Sim4$iter_results <- full_results
  Sim4$concurrent <- TRUE
  file.name2=paste("CorrectionFactors_linear_ar_null_concurrent_n30_rho",rho,"_03102020.csv",sep = "")
  cf=read.table(file.name2,sep=",",h=T)
  Sim4$correction_factors=cf
  results=summarize_results(Sim4)
  
  file.name=paste("Results_linear_ar_pos_concurrent_n30_rho",rho,"_03102020.csv",sep = "")
  print(file.name)
  write.table(results,file.name,sep=",",row.names=F)
