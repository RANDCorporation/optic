...
if (plots){
  ...
  df_sim <- fread("./time_vary_paper/sim_data/sim_results_v3.csv")
  ...
  bias_variance <- function(dd, n_treat, scene){
    ...
    dd <- dd[n_units == n_treat & scenario_name_label == scene,]
    
    plot_scatter <- ggplot(dd, aes(y = model_se_mean, x = model_est_mean, color = model_name)) +
      ...
      geom_errorbarh(aes(xmin = sim_error_025, xmax = sim_error_975), linewidth = 0.5) +
      theme_bw() +
  ...
  args <- expand.grid(n_treat = unique(df_summary$n_units),
                      scene = unique(df_summary$scenario_name_label))
  ...
  ggsave(filename = "./time_vary_paper/plots/time_vary_model_error_v_variance.pdf", plot = marrangeGrob(bias_v_variance_plots, nrow=1, ncol=1, top=NULL), 
  ...
  #                     legend.title = element_text(family = 'sans', size = 14))
  # 
  # #ggsave(filename = "./time_vary_plots/time_vary_sim_bias_ui_2x2.pdf", plot = plot_bias, height = 8.27, width = 11.69, units = "in")
  # 
  # # W/O uncertainty:
  ...
  fig_2 <- ggplot(df_summary_2x2, aes(x = ttt, y = sim_error_50_std, color = model_name)) +
   ...
   ggsave(filename = "./time_vary_paper/plots/fig_2_bias.pdf", 
  ...
  ################
   ...
   ggsave(filename = "./time_vary_paper/plots/time_vary_sim_variance_ui_2x2.pdf", plot = plot_var_2x2, 
  ...
  fig_3 <- ggplot(df_summary_2x2, aes(x = ttt, y = model_se_mean, color = model_name)) +
                  geom_hline(yintercept = 0, linetype = 21) +
                  geom_point(size = 2, shape = 19) +
                  geom_line(size = 0.5, linetype = 2) +
                  facet_wrap(~scenario_name_label) +
  ...
  ggsave(filename = "./time_vary_paper/plots/fig_3_model_se.pdf", plot = fig_3, 
         height = 8.27, width = 11.69, units = "in")
  ...
  fig_4 <- ggplot(df_summary_2x2, aes(x = ttt, y = coverage, color = model_name)) +
              geom_point(size = 2, shape = 19) +
              geom_line(size = 0.5, linetype = 2) +
              facet_wrap(~scenario_name_label) +
              theme_bw() +
  ...
  ggsave(filename = "./time_vary_paper/plots/fig_4_coverage.pdf", plot = fig_4, height = 8.27, width = 11.69, units = "in")
  ...
  fig_5 <- ggplot(df_summary_2x2, aes(x = ttt, y = sim_rmse, color = model_name)) +
            geom_point(size = 2, shape = 19) +
            geom_line(size = 0.5, linetype = 2) +
            facet_wrap(~scenario_name_label) +
            theme_bw() +
  ...
  ggsave(filename = "./time_vary_paper/plots/fig_5_rmse.pdf", 
         plot = fig_5, height = 8.27, width = 11.69, units = "in")
