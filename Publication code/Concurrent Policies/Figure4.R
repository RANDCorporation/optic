# Figure4.R ---------------------------------------------------------------
# 
# Run the new Figure 4 for the revision of the concurrent simulation paper.
# 
...
concurrent_slow_unordered <- read.csv(glue("{dataPath}/slow_unordered.csv"), stringsAsFactors = FALSE)

concurrent_runs <- bind_rows(
  concurrent_slow_unordered %>% mutate(simulation = 'slow_unordered')
)
...
# Figure 4: slow, unordered policies, correctly specified linear models ----

fig_data <- concurrent_runs %>%
  filter(n_units==30,
         specification == "Correctly Specified (Secondary Policy Included)",
         rho==0.9) %>% 
  select(class, coefficient, effect_direction, effect_size, specification,
...
(autoreg_relbias + autoreg_variance) + 
  plot_layout(guides = 'collect')

ggsave(glue('{outputPath}/new_figures/Figure4.png'), width=10, height=5)
