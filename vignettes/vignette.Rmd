...
# Specify three different estimators: Linear fixed-effect w/ and without 
# covariates, along with a linear model using ar-terms and no fixed-effects.

lm_fe_unadj <- optic_model(
    name = "fixed_effect_linear_unadjusted",
    type = "reg",
    call = "lm",
    formula = crude.rate ~ as.factor(year) + as.factor(state) + treatment1_level + treatment2_level,
...
sim_config_noconfound <- optic_simulation(
