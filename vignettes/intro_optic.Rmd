...
OPTIC provides the `summarize_simulation()` function to compute standard performance metrics from simulation results. Given a results data frame and the true effect value, it returns a single-row summary with bias, variance, MSE, rejection rate (Type I error under the null), coverage, Type S error, and the empirical correction factor.

```{r summarize_simulation}
library(dplyr)
...
The individual metric functions (`sim_rejection_rate()`, `sim_bias()`, `sim_coverage()`, etc.) are also exported for custom analyses. See `?sim_rejection_rate` for details.
