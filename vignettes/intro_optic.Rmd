...
# Set number of iterations based on build context
# - Full iterations (1000) for pkgdown website builds
# - Quick iterations (10) for R CMD check and local quick builds
#
# To test locally with full iterations, run in R console:
#   Sys.setenv(BUILD_VIGNETTES_FULL = "true")
#   devtools::build_vignettes()
#
n_iter <- if (identical(Sys.getenv("BUILD_VIGNETTES_FULL"), "true")) {
  1000  # Full iterations for comprehensive results on website
} else {
  10    # Quick iterations for testing and CI checks
}
...
```{r iter_note, echo=FALSE, results='asis'}
if (n_iter < 100) {
  cat("Note: This vignette uses", n_iter, "iterations for demonstration purposes to ensure quick build times. ")
  cat("In practice, we recommend using 300-1000 iterations for reliable results. ")
  cat("With large numbers of iterations, simulations can require several hours to finish. ")
} else {
  cat("Note: This vignette uses", n_iter, "iterations to provide comprehensive results. ")
  cat("For quick testing, you may want to use fewer iterations (e.g., 10-50). ")
}
cat("We suggest using the parallel execution option (`use_future = TRUE`), which can help a great deal for large analyses.")
```
