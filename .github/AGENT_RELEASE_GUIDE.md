...
This guide is for AI agents (Claude Code, GitHub Copilot, etc.) to autonomously manage the release process for the optic R package.

## Overview

You are an AI release manager. Your job is to:
1. Analyze recent changes from git history
2. Draft release notes and NEWS entries
3. Create a release branch using gitflow
4. Run all necessary tests
5. Prepare everything for the user to approve and publish

**DO NOT**: Push tags, merge branches, or submit to CRAN. The user will do these final steps.
...
2. **Special notes**: Any breaking changes or important highlights?
3. **CRAN submission**: Confirm if this is a CRAN release (should be yes for major/minor, no for patch)
...
- **Documentation**: Docs, vignettes, examples
- **Internal**: Refactoring, tests, CI/CD changes (usually not in release notes)
- **Dependencies**: Package dependency changes

### Step 3: Draft Release Notes

Using the analyzed commits, create release notes following this template structure (see `.github/templates/RELEASE_NOTES_TEMPLATE.md`):
...
See [NEWS.md](NEWS.md) for complete details.
```
...
- Replace the "development version" section with the new version
- Add your drafted release notes
- Keep the development section at the top for the next cycle
...
Read current cran-comments.md and update it with:
- Description of changes in this version
- R CMD check results (you'll add this after running checks)
- Notes about any warnings or notes from checks
- Tested platforms
...
- `.github/templates/RELEASE_NOTES_TEMPLATE.md` - Release notes structure
- `.github/templates/NEWS_TEMPLATE.md` - NEWS.md entry format
- `.github/RELEASE_CHECKLIST.md` - Full CRAN checklist
...
**Feature additions:**
- "add", "implement", "introduce", "new"
- Files: New R files in R/, new exports in NAMESPACE

**Bug fixes:**
- "fix", "resolve", "correct", "patch"
- Files: Changes to existing R files

**Breaking changes:**
- "break", "remove", "deprecate", "change API"
- Look for: Removed functions, changed function signatures

**Documentation:**
- "docs", "documentation", "vignette", "readme"
...
**Internal/CI:**
- "refactor", "cleanup", "ci", "workflow"
- Files: .github/, internal functions
...
- Detached HEAD: Ask user to checkout a branch
- Merge conflicts: Report and ask user to resolve

**R CMD check errors:**
- ERRORS: Report in detail, don't proceed
- WARNINGS: Report in detail, ask user if they want to proceed
- NOTES: Document in cran-comments.md, can proceed

**Missing files:**
- NEWS.md: Create from template
- cran-comments.md: Create from template
...
1. **Read commits carefully**: Don't just list commits, synthesize them into user-friendly notes
...
5. **Stop at handoff**: Don't merge or tag - let user review and approve
