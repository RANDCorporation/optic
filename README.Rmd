...
**Simulation test-bed for Longitudinal Causal Inference models (or a better name)**
...
The `optic` package helps you scrutinize candidate causal inference models using **your** longitudinal data.

The recent Diff-in-Diff literature revealed issues with the traditional Diff-in-Diff model, but we found it very difficult to evaluate the performance of causal inference methods using *our data*. Using real-world data, [we find that autoregressive models outperform diff-in-diff models](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/s12874-021-01471-y).

`optic` is an R package we developed to support our work, and now you can use it on your datasets as well. 

The package supports the traditional two-way Diff-in-Diff model, Callway and Santana, Autoregressive models, and multisynth.
